...
use crate::app::modules::user_project::model::{UserProject, NewUserProject};

pub async fn get_user_project_by_user_id(db: &Db, user_id: i32) -> Result<UserProject, diesel::result::Error> {
    let user_project = db.run(move |conn| {
        user_project::table
...
pub async fn create_user_project(db: &Db, new_user_project: NewUserProject) -> Result<UserProject, diesel::result::Error> {
    let user_project = db.run(move |conn| {
